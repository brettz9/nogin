...
const exprt = (on, config) => {
  ...
  on('task', { // Tasks are run in *Node* (unlike commands/custom commands)
    ...
    */
    ...
    async lint (text) {
      ...
      const warnIgnored = true;
      const report = await cli.lintText(text, {
        filePath: 'foo.cjs',
        warnIgnored
      });
  ...
  //   console.log('config.supportFile', config.supportFile);
  //   console.log(
  ...
  //   }
  //
  //   // We catch this or otherwise our test files themselves will
  //   //  be instrumented in place.
  //   // Todo: This apparently doesn't work because we need the imports
  //   //   *within* the test files (and they are not reported)?
  //   if ([
